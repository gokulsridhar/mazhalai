...
from mazhalai.lexer import Lexer, TokenStream
from mazhalai.parser import Parser
from mazhalai.errors import MazhalaiSyntaxError, report_syntax_error
from mazhalai.utils import print_ast, print_tokens, print_env

""" 
 Enter the keywords you are going to use in the below section

 "Warning: Do not change the name of the keywords"  
"""
...
def add_builtins(env):
    builtins = {
        print_keyword: (['value'], lambda args, e: print(args['value'])),
        len_keyword: (['iter'], lambda args, e: len(args['iter'])),
        slice_keyword: (['iter', 'start', 'stop'], lambda args, e: list(args['iter'][args['start']:args['stop']])),
        str_keyword: (['in'], lambda args, e: str(args['in'])),
        int_keyword: (['in'], lambda args, e: int(args['in'])),
    }
...
def evaluate_env(s, env, verbose=False):
    ...
    except MazhalaiSyntaxError as err:
